...
for country, avg in avg_words_per_country.items():
    print(f"{country}: {avg} words")
...
duplicates = df.groupby("duplicate_title").filter(lambda x: x["country"].nunique() > 1)
if len(duplicates)>0:
...
if total_headlines > 0:
    ...
    recent_percentage = older_percentage = 0
...
print(f"{shortest_country} has the shortest headline on average, {shortest_avg:.2f}")